...
class Database:
    ...
    def _wait_for_database(self, max_retries=30, retry_interval=2):
        """等待数据库可用，带重试机制"""
        self.logger.info("正在等待数据库启动...")
        
        for attempt in range(1, max_retries + 1):
            try:
                conn = self._get_connection(test_connection=True)
                if conn:
                    self.logger.info(f"数据库连接成功（第{attempt}次尝试）")
                    conn.close()
                    return
            except Exception as e:
                if attempt < max_retries:
                    self.logger.warning(f"数据库连接失败（第{attempt}次尝试）: {e}，{retry_interval}秒后重试...")
                    time.sleep(retry_interval)
                else:
                    self.logger.error(f"数据库连接失败，已达到最大重试次数({max_retries}): {e}")
                    self.logger.error(traceback.format_exc())
                    # 最后一次失败后，记录错误但不抛出异常，允许应用继续运行
                    # 这样在运行时可能会恢复连接
