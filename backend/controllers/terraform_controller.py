...
class TerraformController:
    ...
    def generate_terraform_code(self, user_description, mermaid_code):
        ...
        try:
            ...
            # 如果包含火山云和ECS关键词，添加火山云ECS数据源要求
            if is_volcengine_ecs:
                self.logger.info("检测到火山云/火山引擎和ECS关键词，添加ECS数据源")
                system_prompt += """
            
            IMPORTANT FOR VOLCENGINE ECS INSTANCES:
...
'''
            ...
            # 如果是火山云ECS配置，但没有相应的数据源，手动添加
            if is_volcengine_ecs:
                # 检查生成的代码中是否存在相关配置
                has_instance_types = 'data "volcengine_ecs_instance_types" "available"' in terraform_code
                has_images = 'data "volcengine_images" "centos"' in terraform_code
                
                # 检查ECS实例是否使用了数据源引用
                if 'resource "volcengine_ecs_instance"' in terraform_code:
                    self.logger.info("检测到火山云ECS实例，应用固定的实例类型和镜像ID")
                    
                    # 检查和替换实例类型
                    if 'instance_type = "ecs.c3il.large"' not in terraform_code:
                        # 正则表达式，匹配ECS实例中的instance_type行
                        instance_type_pattern = r'(resource\s+"volcengine_ecs_instance"\s+".+?"\s+{.*?)(instance_type\s+=\s+".+?")(.*?})'
                        terraform_code = re.sub(instance_type_pattern, r'\1instance_type = "ecs.c3il.large"\3', terraform_code, flags=re.DOTALL)
                    
                    # 检查和替换镜像ID
                    if 'image_id = "image-aagd56zrw2jtdro3bnrl"' not in terraform_code:
                        # 正则表达式，匹配ECS实例中的image_id行
                        image_id_pattern = r'(resource\s+"volcengine_ecs_instance"\s+".+?"\s+{.*?)(image_id\s+=\s+".+?")(.*?})'
                        terraform_code = re.sub(image_id_pattern, r'\1image_id = "image-aagd56zrw2jtdro3bnrl"\3', terraform_code, flags=re.DOTALL)
                    
                    # 检查和设置系统卷类型
                    if 'system_volume_type = "ESSD_PL0"' not in terraform_code:
                        # 检查ECS实例中是否已经有system_volume_type
                        system_volume_type_pattern = r'resource\s+"volcengine_ecs_instance"\s+".+?"\s+{.*?system_volume_type\s+=\s+".+?".*?}'
                        if not re.search(system_volume_type_pattern, terraform_code, re.DOTALL):
                            # 如果没有，添加system_volume_type
                            ecs_instance_pattern = r'(resource\s+"volcengine_ecs_instance"\s+".+?"\s+{.*?)(image_id\s+=\s+".+?")(.*?})'
                            terraform_code = re.sub(ecs_instance_pattern, r'\1\2\n  system_volume_type = "ESSD_PL0"\3', terraform_code, flags=re.DOTALL)
                        else:
                            # 如果已经有，替换为推荐值
                            system_volume_type_replace_pattern = r'(resource\s+"volcengine_ecs_instance"\s+".+?"\s+{.*?)(system_volume_type\s+=\s+".+?")(.*?})'
                            terraform_code = re.sub(system_volume_type_replace_pattern, r'\1system_volume_type = "ESSD_PL0"\3', terraform_code, flags=re.DOTALL)
    ...
    def _fix_terraform_code(self, original_code, error_message, tf_file_path):
        ...
        try:
            ...
            # 正确方式初始化OpenAI客户端，使用自定义base_url
            client = openai.OpenAI(
                api_key=api_key,
                base_url=api_base_url
            )
