...
"""
                ...
                # 创建部署状态文件，用于前端轮询检查状态
                status_file = f"{tf_dir}/status.json"
                try:
                        ...
                        with open(log_file, 'a') as f:
                            ...
                            try:
                                ...
                                if os.path.exists(deploy_dir):
                                    ...
                                    dot_cmd = ["dot", "-Tpng", "-o", f"{deploy_dir}/graph.png"]
                                    ...
                                    if dot_process.returncode == 0:
                                        controller_ref.logger.info(f"✅ 成功生成拓扑图: {deploy_dir}/graph.png")
...
'''
                ...
                # 创建状态文件，用于前端轮询检查状态
                status_file = f"{deploy_dir}/status.json"
                try:
                    with open(status_file, 'w') as f:
                        json.dump({
                            'status': 'in_progress',
...
'''
    ...
    def get_deployment_status(self):
        ...
        try:
            ...
            # 构建状态文件路径
            status_file = f"deploy/{deploy_id}/status.json"
    ...
    def handle_execute_deploy(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if (status == 'completed' or status == 'success') and resources.get('vpc_id'):
                ...
                # 查找错误日志
                log_path = f"deploy/{deploy_id}/deploy.log"
                error_path = f"deploy/{deploy_id}/error.txt"
                error_msg = "部署过程中发生错误"
                tf_error = ""
                error_lines = []
