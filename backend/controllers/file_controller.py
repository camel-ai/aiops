...
class FileController:
    ...
    def get_deployment_file(self) -> Any:
        ...
        try:
            ...
            # 根据部署类型确定基础目录
            if deploy_type == 'query':
                base_dir = os.path.join(app_root, 'query')
            elif deploy_type == 'template':
                # 对于模板部署类型，使用deployments目录
                base_dir = os.path.join(app_root, 'deployments')
            else:
                # 默认为deploy类型
                base_dir = os.path.join(app_root, 'deploy')
                ...
                if deploy_type == 'template' and filename.lower() == 'graph.png':
                    ...
                    if os.path.exists(template_dir):
                        ...
                        try:
                            import subprocess
                            # 使用绝对路径确保命令正确执行
                            cmd = f"cd {template_dir} && terraform graph -type=plan | dot -Tpng > \"{os.path.abspath(graph_path)}\""
            ...
            if not os.path.exists(abs_path):
                self.logger.error(f"文件不存在，绝对路径: {abs_path}")
                return jsonify({"success": False, "message": "文件不存在"}), 404
                
            self.logger.info(f"文件存在，大小: {os.path.getsize(abs_path)} 字节")
