...
<template>
          ...
          >
            ...
            <div class="message-wrapper">
              <div v-if="message.type === 'system'" class="message-avatar">
                <img src="@/assets/aiops.png" alt="AI助手" class="avatar-image" />
              </div>
              <div class="message-content">
                <div v-html="message.content"></div>
                <!-- 流式输出指示符 -->
                <span v-if="message.is_streaming" class="streaming-indicator">
            ...
            <div v-if="message.is_diagram && message.mermaid_code" class="diagram-container">
              ...
              <TerraformDeployment 
                ...
                :selected-project="getProjectById(selectedProject)"
                :selected-cloud="getCloudById(selectedCloud)"
            ...
            <!-- 产品选择区域 -->
            <div v-if="message.product_selection" class="product-selection-container">
              <div class="product-header">请选择要查询的云产品:</div>
              <div class="product-options">
                <div v-for="product in message.cloud_products" :key="product.id" class="product-option">
                  <el-checkbox 
                    v-model="selectedProducts" 
                    :label="product.id"
                    :disabled="isAllProductSelected && product.id !== 'all'"
              ...
              <el-button 
                type="primary" 
                @click="confirmProductSelection(message.metadata)" 
  ...
  >
    <div class="project-dialog-content" style="min-height: 400px; padding: 10px;">
      <!-- 直接使用引入的组件 -->
      <ProjectAdd @project-created="handleProjectCreated" />
    </div>
  </el-dialog>
...
export default {
  ...
  setup() {
    ...
    const isAiMode = ref(false)  // 是否处于AI描述模式
    let streamAbortController = null  // 用于取消流式请求
    const chatMessagesRef = ref(null)
    let welcomeTypeInterval = null  // 用于管理欢迎消息打字机效果的interval
    
    // 生成个性化欢迎消息
    const getWelcomeText = () => {
      const username = currentUser.value?.username || '朋友'
      return `hi, <strong style="color: #409EFF;">${username}</strong>, 很高兴和你见面，欢迎来到《aiops》!我是你私人运维小助手，在你运维过程中，是否也曾对如下这些问题感到疑惑或者好奇？

1.什么叫<strong>「aiops」</strong>? 
2.每天要运维的云有哪些？ta 们的<strong>「登录」</strong>和<strong>「管理」</strong>容易么？
3.传统的CMP平台与云portal真的会<strong>「思考」</strong>吗？和AIOPS有什么不同？
4.运维过程中面对如此多的<strong>「key」</strong>我该怎么办？
5.我部署过程中需要开如此多的<strong>「窗口」</strong>协同工作么？
6.每次<strong>「部署失败」</strong>都需要<strong>「重新开始」</strong>么？
7.如何<strong>「雇佣AI」</strong>帮我干1天的活？
8.作为<strong>「aiops」</strong>运维小助手，我能干些啥？

tips:首先，如果是基于项目的部署，你需要先点击<strong><span style="color: #409EFF; cursor: pointer; text-decoration: underline;" onclick="window.openProjectDialog()">「新增项目」</span></strong>来添加你的第一个项目噢~`
    }
    ...
    const projectSelectKey = ref(0) // 用于强制重新渲染项目选择下拉菜单
    ...
    const isAllProductSelected = computed(() => selectedProducts.value.includes('all'))
    ...
    const stopStreaming = () => {
      ...
      if (messages.value.length > 0) {
        ...
        if (lastMessage.type === 'system' && lastMessage.is_streaming) {
    ...
    const sendMessageStream = async (message) => {
      ...
      try {
        ...
        const token = localStorage.getItem('token')
        if (!token) {
          console.error('流式聊天：未找到认证令牌')
          ElMessage.error('请先登录')
          router.push('/login')
          return
        }
        
        console.log('流式聊天：开始处理消息', message)
        
        // 检查是否应该使用普通聊天（@ai、@查询、@部署、@模版部署等特殊命令）
        if (message.startsWith('@ai') || message.includes('@查询') || message.includes('@部署') || message.includes('@模版部署')) {
          // 这些命令不适合流式输出，使用普通方式
          console.log('流式聊天：检测到特殊命令，重定向到普通聊天')
        ...
        // 为了保持流式聊天的效果，我们不在前端保存消息，而是让后端处理
        ...
        console.log('流式聊天：发送请求数据', requestData)
        ...
        const response = await fetch('/api/chat/stream', {
          ...
          signal: streamAbortController.signal  // 添加取消信号
        })
        
        console.log('流式聊天：收到响应状态', response.status, response.statusText)
        
        if (!response.ok) {
          // 根据不同的HTTP状态码进行不同处理
          if (response.status === 401) {
            console.error('流式聊天：认证失败，可能需要重新登录')
            ElMessage.error('认证已过期，请重新登录')
            localStorage.removeItem('token')
            router.push('/login')
            return
          } else if (response.status === 403) {
            console.error('流式聊天：权限不足')
            ...
            for (const line of lines) {
              if (line.startsWith('data: ')) {
                try {
                  ...
                  // 处理错误
                  if (data.error) {
                    console.error('流式聊天：收到错误响应', data.error)
                    systemMessage.content = `错误: ${data.error}`
                    systemMessage.is_streaming = false
                    break
                  }
                  
                  // 处理重定向到普通聊天
                  if (data.redirect_to_normal) {
                    console.log('流式聊天：服务器请求重定向到普通聊天')
                    // 移除流式消息，使用普通方式
                    messages.value.pop()
                    userInput.value = message
                    await sendMessage()
                  ...
                  if (data.content !== undefined && !data.done) {
                    systemMessage.content += data.content
                    console.log(`流式聊天：更新内容，当前长度: ${systemMessage.content.length}`)
                    
                    // 强制触发响应式更新
                    messages.value[messages.value.length - 1] = { ...systemMessage }
                  ...
                  // 处理完成信号
                  if (data.done) {
                    console.log('流式聊天：收到完成信号')
                    systemMessage.is_streaming = false
                    if (data.full_content) {
                      systemMessage.content = data.full_content
                    }
                    
                    // 处理JSON内容
                    if (data.has_json && data.json_content) {
                      systemMessage.has_json = true
                      systemMessage.json_content = data.json_content
                    }
                  ...
                  console.error('流式聊天：解析SSE数据失败', e, line)
                }
              }
            }
          }
          
          console.log(`流式聊天：处理完成，共处理 ${chunkCount} 个数据块`)
        ...
        if (error.name === 'AbortError') {
          console.log('流式聊天：用户主动取消请求')
          return
        }
        
        console.error('流式聊天：发送消息失败', error)
        ...
        // 显示错误消息，但不要直接跳转到登录页面
        let errorMessage = '抱歉，发送消息时出现错误。请稍后再试。'
    ...
    const sendMessage = async () => {
      ...
      if (isAiMode.value) {
        message = `@ai ${userInput.value}`
        // 重置AI模式和输入框提示语
        isAiMode.value = false
        nextTick(() => {
          const inputElement = document.querySelector('.chat-input textarea')
          if (inputElement) {
            inputElement.placeholder = '输入消息，按enter发送，shift+enter换行'
          }
        })
      }
    ...
    const handleProjectChange = (projectId) => {
      ...
      if (projectList.value.length === 0) {
        ...
        fetchProjects().then(() => {
          ...
          if (validProject) {
            ...
            // 保存到localStorage
            localStorage.setItem('selectedProject', numericProjectId.toString());
      ...
      // 将选定的项目ID存储到Vuex store和localStorage
      store.dispatch('setSelectedProject', numericProjectId);
      localStorage.setItem('selectedProject', numericProjectId.toString());
      console.log(`成功设置项目: ID=${numericProjectId}, 名称=${validProject.name}`);
    ...
    const handleCloudChange = (cloudId) => {
      ...
      if (cloudList.value.length === 0) {
        ...
        fetchClouds().then(() => {
          ...
          if (validCloud) {
            ...
            // 保存到localStorage
            localStorage.setItem('selectedCloud', numericCloudId.toString());
      ...
      // 将选定的云ID存储到Vuex store和localStorage
      store.dispatch('setSelectedCloud', numericCloudId);
      localStorage.setItem('selectedCloud', numericCloudId.toString());
      console.log(`成功设置云平台: ID=${numericCloudId}, 名称=${validCloud.name}`);
    ...
    const confirmQuery = async (queryInfo) => {
      ...
      try {
        ...
        // 如果region是all，需要传递actual_regions
        if (queryInfo.region === 'all' && queryInfo.actual_regions) {
          queryData.actual_regions = queryInfo.actual_regions
        }
        ...
        if (error.code === 'ECONNABORTED' || error.message.includes('timeout')) {
          errorMessage = '查询执行超时，请检查网络连接或稍后再试。'
    ...
    const fetchChatHistory = async () => {
      try {
        ...
        // 检查是否有清屏时间点
        const clearTime = localStorage.getItem('chat_clear_time')
        let url = '/api/chat/history'
        if (clearTime) {
          url += `?clear_time=${encodeURIComponent(clearTime)}`
          console.log('🔍 清屏时间点存在:', clearTime)
          console.log('🔍 请求URL:', url)
        } else {
          console.log('🔍 没有清屏时间点，加载所有历史')
        }
        ...
        if (response.data && response.data.success && response.data.messages) {
          ...
          if (response.data.messages.length > 0) {
            ...
            console.log('已加载聊天历史记录:', messages.value.length, '条消息')
    ...
    const showWelcomeMessage = () => {
      ...
      console.log('开始显示欢迎消息')
      console.log('当前用户:', currentUser.value)
      console.log('用户名:', currentUser.value?.username)
      console.log('welcomeText长度:', welcomeText.length)
      
      // 清除之前的打字机interval
      if (welcomeTypeInterval) {
        clearInterval(welcomeTypeInterval)
        welcomeTypeInterval = null
      }
      ...
      // 打字机效果
      let currentIndex = 0
      welcomeTypeInterval = setInterval(() => {
        if (currentIndex < welcomeText.length) {
          // 确保字符正确显示
          const currentText = welcomeText.substring(0, currentIndex + 1)
          ...
          if (currentIndex % 10 === 0) {
            console.log(`打字机进度: ${currentIndex}/${welcomeText.length}`)
          }
        } else {
          // 打字完成，移除流式标识
          console.log('打字机效果完成')
    ...
    const clearScreen = () => {
      ...
      // 记录清屏时间点到localStorage
      const clearTime = new Date().toISOString()
      localStorage.setItem('chat_clear_time', clearTime)
      
      // 清除可能存在的打字机interval
      if (welcomeTypeInterval) {
        clearInterval(welcomeTypeInterval)
        welcomeTypeInterval = null
        console.log('清理之前的打字机interval')
      }
      ...
      console.log('清屏完成，设置清屏时间点:', clearTime)
      ElMessage.success('屏幕已清空')
    }

    // 打开新增项目弹窗
    const openProjectDialog = () => {
      console.log('从欢迎消息点击打开新增项目弹窗')
      showProjectDialog.value = true
      // 同时更新路由，保持状态一致
      router.push('/workspace/project/add')
    }

    // 打开API-KEY管理弹窗
    const openApiKeyDialog = () => {
      console.log('从聊天消息点击打开API-KEY管理弹窗')
      showApiKeyDialog.value = true
      // 同时更新路由，保持状态一致
      router.push('/workspace/apikey')
    }

    // 触发命令发送
    const triggerCommand = (command) => {
      console.log('从聊天消息点击触发命令:', command)
    ...
    // 触发AI模式
    const triggerAiMode = () => {
      console.log('从聊天消息点击触发AI模式')
      // 设置输入框提示文字为AI描述模式
      const inputElement = document.querySelector('.chat-input textarea')
      if (inputElement) {
        inputElement.placeholder = '输入你想让ai部署的云架构的描述，点+上传附件后，也可以从手稿自动生成云架构的描述'
        inputElement.focus()
      }
      // 清空当前输入
      userInput.value = ''
      // 设置一个标志，表明接下来的输入是AI描述
      isAiMode.value = true
    }
    ...
    const confirmRegionSelection = async (metadata) => {
      ...
      try {
        ...
        if (response.data) {
          ...
          // 处理产品选择响应
          if (response.data.product_selection) {
            systemMessage.product_selection = true
            systemMessage.cloud_products = response.data.cloud_products
          }
    ...
    const handleProductChange = (product) => {
      console.log('产品选择变化:', product, '当前选择:', selectedProducts.value)
      
      // 使用nextTick确保DOM更新后再处理逻辑
      nextTick(() => {
        if (product.id === 'all') {
          // 如果点击的是ALL选项
          ...
          // 如果点击的是其他产品选项
          if (selectedProducts.value.includes('all')) {
            // 如果当前包含ALL，移除ALL，只保留当前选择的产品
            selectedProducts.value = selectedProducts.value.filter(id => id !== 'all')
          }
        }
        
        console.log('产品选择变化后:', selectedProducts.value)
      })
    }
    
    // 确认产品选择
    const confirmProductSelection = async (metadata) => {
      console.log('确认产品选择:', selectedProducts.value, metadata)
      ...
      try {
        ...
        console.log('发送产品选择数据:', JSON.stringify(productData))
        
        // 发送产品选择数据
        const token = localStorage.getItem('token')
        const response = await axios.post('/api/chat/confirm-products',
          productData,
          {
            headers: {
              'Content-Type': 'application/json',
        ...
        console.log('产品选择响应:', response.data)
        ...
        if (response.data) {
          ...
          // 添加选择的产品信息
          if (response.data.selected_products) {
            systemMessage.selected_products = response.data.selected_products
          }
          ...
          messages.value.push({
            type: 'system',
            content: '产品选择完成，但未返回结果',
            selected_products: selectedProducts.value,
            metadata: metadata
          })
        }
      } catch (error) {
        console.error('确认产品选择失败:', error)
        messages.value.push({
          type: 'system',
          content: `产品选择失败：${error.message || '未知错误'}`,
          is_error: true
        })
      } finally {
        loading.value = false
        // 确保产品选择页面完全关闭但不重置选择的产品值
        const lastMessage = messages.value[messages.value.length - 1]
        if (lastMessage) {
          lastMessage.product_selection = false
        }
        scrollToBottom()
      }
    }
    ...
    onMounted(() => {
      ...
      // 将全局函数绑定到window对象，以便从HTML内容中调用
      window.openProjectDialog = openProjectDialog;
      window.openApiKeyDialog = openApiKeyDialog;
      window.triggerCommand = triggerCommand;
      window.triggerAiMode = triggerAiMode;
      ...
      Promise.all([fetchProjects(), fetchClouds()])
        .then(() => {
          ...
          // 优先从localStorage恢复用户选择，然后才是Vuex store
          const localStorageProjectId = localStorage.getItem('selectedProject');
          const localStorageCloudId = localStorage.getItem('selectedCloud');
          const storedProjectId = localStorageProjectId || store.state.selectedProject;
          const storedCloudId = localStorageCloudId || store.state.selectedCloud;
          
          console.log('恢复用户选择状态:', { 
    ...
    onUnmounted(() => {
      ...
      if (welcomeTypeInterval) {
        clearInterval(welcomeTypeInterval)
        welcomeTypeInterval = null
        console.log('清理欢迎消息打字机interval')
      }
      
      // 清理全局函数
      if (window.openProjectDialog) {
        delete window.openProjectDialog;
        console.log('清理全局函数 openProjectDialog');
      }
      if (window.openApiKeyDialog) {
        delete window.openApiKeyDialog;
        console.log('清理全局函数 openApiKeyDialog');
      }
      if (window.triggerCommand) {
        delete window.triggerCommand;
        console.log('清理全局函数 triggerCommand');
      }
      if (window.triggerAiMode) {
        delete window.triggerAiMode;
        console.log('清理全局函数 triggerAiMode');
      }
    ...
    const handleProjectCreated = async (newProject) => {
      console.log('收到项目创建成功事件:', newProject);
      ...
      // 强制更新项目选择下拉菜单
      projectSelectKey.value += 1;
      
      // 如果新项目有ID，自动选择它
      if (newProject && newProject.id) {
        selectedProject.value = newProject.id;
        store.dispatch('setSelectedProject', newProject.id);
        localStorage.setItem('selectedProject', newProject.id.toString());
        console.log('自动选择新创建的项目:', newProject.name);
      } else if (!selectedProject.value && projectList.value.length > 0) {
        // 如果没有新项目ID，选择第一个项目
        selectedProject.value = projectList.value[0].id;
        store.dispatch('setSelectedProject', selectedProject.value);
        localStorage.setItem('selectedProject', selectedProject.value.toString());
        console.log('自动选择第一个项目:', projectList.value[0].name);
      }
      
      // 使用nextTick确保DOM更新
      nextTick(() => {
        console.log('项目列表DOM已更新，projectSelectKey:', projectSelectKey.value);
      });
    };
    ...
    return {
      ...
      handleProjectChange,
      handleCloudChange,
      getProjectById: (id) => projectList.value.find(p => p.id === id) || { id: 0, name: '未命名项目' },
      getCloudById: (id) => cloudList.value.find(c => c.id === id) || { id: 0, name: '未知云平台' },
      deploymentStatus,
      deploymentSummary,
      activeCollapse,
      getProjectName,
...
.product-option .el-checkbox {
  width: 100%;
}

.product-option .el-checkbox__label {
  display: flex;
  flex-direction: column;
  align-items: flex-start;
  width: 100%;
