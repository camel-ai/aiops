<template>
  <div class="terraform-container">
    ...
    <!-- 部署状态显示区域 - 不使用对话框，直接在组件内显示 -->
    <div v-if="isDeploying || deploymentStatus" class="deployment-status-inline">
      <div class="status-header">
        <h4>部署状态 - ID: {{ currentDeployId }}</h4>
...
export default {
  ...
  setup(props, { emit }) {
      ...
      try {
        ...
        if (codeParts > 1) {
          ...
          const firstPart = {
            ...
            project_name: props.selectedProject?.name || '未命名项目',
            cloud_id: props.selectedCloud?.id || 0,
            cloud_name: props.selectedCloud?.name || '未知云平台'
          };
    ...
    const stopDeployment = async () => {
      if (!currentDeployId.value) {
        ElMessage.warning('没有正在进行的部署任务');
        return;
      }
      
      try {
        // 调用后端API停止部署
        const response = await axios.post(`/api/terraform/deploy/stop`, {
          deploy_id: currentDeployId.value
        });
        ...
        if (response.data.success) {
          ...
          // 重置状态
          isDeploying.value = false;
          deploymentStatus.value = 'stopped';
          emit('deploy-failed', '用户手动停止部署');
        } else {
          throw new Error(response.data.message || '停止部署失败');
        }
      } catch (error) {
        console.error('停止部署失败:', error);
        ElMessage.error(`停止部署失败: ${error.message}`);
      }
    };
...
.deployment-summary h5, .deployment-summary h6 {
  margin: 0 0 8px 0;
  font-size: 14px;
  color: #333;
}
